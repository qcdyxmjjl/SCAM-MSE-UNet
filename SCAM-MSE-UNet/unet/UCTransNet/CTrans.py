...
class Attention_org(nn.Module):
    ...
    def forward(self, emb1,emb2,emb3,emb4, emb_all):
        ...
        multi_head_Q1 = multi_head_Q1.transpose(-1, -2) if multi_head_Q1 is not None else None
        multi_head_Q2 = multi_head_Q2.transpose(-1, -2) if multi_head_Q2 is not None else None
        multi_head_Q3 = multi_head_Q3.transpose(-1, -2) if multi_head_Q3 is not None else None
        multi_head_Q4 = multi_head_Q4.transpose(-1, -2) if multi_head_Q4 is not None else None

        attention_scores1 = torch.matmul(multi_head_Q1, multi_head_K) if multi_head_Q1 is not None else None
        attention_scores2 = torch.matmul(multi_head_Q2, multi_head_K) if multi_head_Q2 is not None else None
        attention_scores3 = torch.matmul(multi_head_Q3, multi_head_K) if multi_head_Q3 is not None else None
        attention_scores4 = torch.matmul(multi_head_Q4, multi_head_K) if multi_head_Q4 is not None else None

        attention_scores1 = attention_scores1 / math.sqrt(self.KV_size) if attention_scores1 is not None else None
        attention_scores2 = attention_scores2 / math.sqrt(self.KV_size) if attention_scores2 is not None else None
        attention_scores3 = attention_scores3 / math.sqrt(self.KV_size) if attention_scores3 is not None else None
        attention_scores4 = attention_scores4 / math.sqrt(self.KV_size) if attention_scores4 is not None else None

        attention_probs1 = self.softmax(self.psi(attention_scores1)) if attention_scores1 is not None else None
        attention_probs2 = self.softmax(self.psi(attention_scores2)) if attention_scores2 is not None else None
        attention_probs3 = self.softmax(self.psi(attention_scores3)) if attention_scores3 is not None else None
        attention_probs4 = self.softmax(self.psi(attention_scores4)) if attention_scores4 is not None else None
        # print(attention_probs4.size())

        if self.vis:
            weights =  []
            weights.append(attention_probs1.mean(1) if attention_probs1 is not None else None)
            weights.append(attention_probs2.mean(1) if attention_probs2 is not None else None)
            weights.append(attention_probs3.mean(1) if attention_probs3 is not None else None)
            weights.append(attention_probs4.mean(1) if attention_probs4 is not None else None)
        else: weights=None

        attention_probs1 = self.attn_dropout(attention_probs1) if attention_probs1 is not None else None
        attention_probs2 = self.attn_dropout(attention_probs2) if attention_probs2 is not None else None
        attention_probs3 = self.attn_dropout(attention_probs3) if attention_probs3 is not None else None
        attention_probs4 = self.attn_dropout(attention_probs4) if attention_probs4 is not None else None

        multi_head_V = multi_head_V.transpose(-1, -2)
        context_layer1 = torch.matmul(attention_probs1, multi_head_V) if attention_probs1 is not None else None
        context_layer2 = torch.matmul(attention_probs2, multi_head_V) if attention_probs2 is not None else None
        context_layer3 = torch.matmul(attention_probs3, multi_head_V) if attention_probs3 is not None else None
        context_layer4 = torch.matmul(attention_probs4, multi_head_V) if attention_probs4 is not None else None

        context_layer1 = context_layer1.permute(0, 3, 2, 1).contiguous() if context_layer1 is not None else None
        context_layer2 = context_layer2.permute(0, 3, 2, 1).contiguous() if context_layer2 is not None else None
        context_layer3 = context_layer3.permute(0, 3, 2, 1).contiguous() if context_layer3 is not None else None
        context_layer4 = context_layer4.permute(0, 3, 2, 1).contiguous() if context_layer4 is not None else None
        context_layer1 = context_layer1.mean(dim=3) if context_layer1 is not None else None
        context_layer2 = context_layer2.mean(dim=3) if context_layer2 is not None else None
        context_layer3 = context_layer3.mean(dim=3) if context_layer3 is not None else None
        context_layer4 = context_layer4.mean(dim=3) if context_layer4 is not None else None

        O1 = self.out1(context_layer1) if context_layer1 is not None else None
        O2 = self.out2(context_layer2) if context_layer2 is not None else None
        O3 = self.out3(context_layer3) if context_layer3 is not None else None
        O4 = self.out4(context_layer4) if context_layer4 is not None else None
        O1 = self.proj_dropout(O1) if O1 is not None else None
        O2 = self.proj_dropout(O2) if O2 is not None else None
        O3 = self.proj_dropout(O3) if O3 is not None else None
        O4 = self.proj_dropout(O4) if O4 is not None else None
...
class Block_ViT(nn.Module):
    ...
    def forward(self, emb1,emb2,emb3,emb4):
        ...
        x1 = x1 + org1 if x1 is not None and org1 is not None else None
        x2 = x2 + org2 if x2 is not None and org2 is not None else None
        x3 = x3 + org3 if x3 is not None and org3 is not None else None
        x4 = x4 + org4 if x4 is not None and org4 is not None else None
